...
class TextbookAgent:
    ...
    def generate_practice_material(self, image_bytes: bytes, difficulty: str,
                                   grade_levels: List[str] = None) -> str:
        """
        Generate educational content based on difficulty level
        ...
        Returns:
            Clean plain text string ready for display
        """
    ...
    # NEW: A single dynamic prompt builder
    def _create_prompt(self, level_name: str, language_style: str, section1_title: str,
                       ...
                       section3_title: str, section3_desc: str) -> str:
        """Creates a dynamically structured prompt for a specific difficulty level."""
        return f"""
Analyze the provided image. Based on its content, generate educational material with the following specifications.

CRITICAL INSTRUCTION: You MUST generate content for the specified difficulty level: {level_name}.

MANDATORY LANGUAGE STYLE:
{language_style}

MANDATORY STRUCTURE:
Create content with these THREE sections, using the exact titles provided.
...
FINAL CHECK: Ensure the vocabulary, tone, and complexity perfectly match the target {level_name} audience.
"""
    ...
    def generate_multiple_difficulty_levels(self, image_bytes: bytes) -> Dict[str, str]:
        """Generate materials for all difficulty levels"""
        results = {}
        
        for difficulty in ["basic", "intermediate", "advanced"]:
            results[difficulty] = self.generate_practice_material(image_bytes, difficulty)
